...
public class CartService {
    ...
    public Boolean addCart(List<Cart> carts) {
        try {
            ...
            for (Cart cart : carts) {
                ...
                //获取用户信息
                UserInfo loginUser = LoginInterceptor.getUserInfo();
                ...
                if (haskey) {
                    ...
                    cart = JsonUtils.toBean(carString, Cart.class);
    ...
    public List<Cart> loadCart() {
        ...
        List<Cart> carts = stringCarts.stream().map(cart -> JsonUtils.toBean(cart.toString(), Cart.class)).collect(Collectors.toList());
        return carts;
    }
    ...
    public Boolean updateCart(Cart cart) {
        try {
            ...
            Cart parse = JsonUtils.toBean(stringCart, Cart.class);
            parse.setNum(cart.getNum());
            cartMap.put(cart.getSkuId().toString(), JsonUtils.toString(parse));
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
